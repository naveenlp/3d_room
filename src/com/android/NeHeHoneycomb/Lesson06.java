...
public class Lesson06 implements Renderer {
	...
	public float anglerot[] = new float[3];				//X Rotation ( NEW )
	//Y Rotation ( NEW )
		//Z Rotation ( NEW )
	...
	boolean isRotationMatrixReady = false;
	public void onDrawFrame(GL10 gl) {
		//Clear Screen And Depth Buffer
		gl.glClear(GL10.GL_COLOR_BUFFER_BIT | GL10.GL_DEPTH_BUFFER_BIT);	
		gl.glLoadIdentity();					//Reset The Current Modelview Matrix
		gl.glScalef(4.1f, 4.1f, 4.1f);
		//Drawing
		//gl.glTranslatef(0.0f, 0.0f, -5.0f);		//Move 5 units into the screen
		//gl.glScalef(0.8f, 0.8f, 0.8f); 			//Scale the Cube to 80 percent, otherwise it would be too large for the screen
		
		//Rotate around the axis based on the rotation matrix (rotation, x, y, z)
		//gl.glRotatef(anglerot[0], 1.0f, 0.0f, 0.0f);	//X
		//gl.glRotatef(anglerot[1], 0.0f, 1.0f, 0.0f);	//Y
		//gl.glRotatef(anglerot[2], 0.0f, 0.0f, 1.0f);	//Z
